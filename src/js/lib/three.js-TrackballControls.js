...
THREE.TrackballControls = function ( object, domElement ) {
	...
	this.touchOffset = function( event, touch ) {
		var box = event.target.getBoundingClientRect();
		return { X: touch.pageX - box.left - window.pageXOffset, Y: touch.pageY - box.top - window.pageYOffset };
	}
	...
	function mousemove( event ) {
		...
		if ( _state === STATE.ROTATE && !_this.noRotate ) {
			...
			var offset = _this.offset(event);
			_rotateEnd = _this.getMouseProjectionOnBall( offset.X, offset.Y );
	...
	function touchstart( event ) {
		...
		switch ( event.touches.length ) {
			...
			case 1:
				...
				var offset = _this.touchOffset(event, event.touches[ 0 ]);
				_rotateStart = _rotateEnd = _this.getMouseProjectionOnBall( offset.X, offset.Y );
				break;
